...
export default function AddSessionByClassroomCmp(props) {
  ...
  const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
      if(props.toadd){
        db.classrooms.get(Number.parseInt(props.toadd.classroom_id)).then(res=>{
          if(res){
          setClassroom(res);
          setLoading(false);
        }
    ...
    useEffect(() => {
      setInputs({
        ...
        class_id:'',
        teacher_id:'',
        classroom_id:props.toadd.classroom_id,
