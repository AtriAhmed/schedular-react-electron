...
export default function AddSessionByClassCmp(props) {
  ...
  const submitSession = async (e) => {
    ...
    if (!inputs.end_time) {
      setErrors((errors) => ({
        ...errors,
        end_time: 'Vous devez sélectionner un temps du fin',
      }));
    }
    if (!inputs.subject_id) {
      setErrors((errors) => ({
        ...errors,
        subject_id: 'Vous devez sélectionner une matière',
      }));
    }
    if (!inputs.teacher_id) {
      setErrors((errors) => ({
        ...errors,
        teacher_id: 'Vous devez sélectionner un prof',
      }));
    }
    if (!inputs.classroom_id) {
      setErrors((errors) => ({
        ...errors,
        classroom_id: 'Vous devez sélectionner une salle',
      }));
    }
    if (
      inputs.end_time &&
      ...
      db.sessions
        ...
        .then(async () => {
          if (classNotFree) {
            Swal.fire(
              'Erreur',
              `Classe ${classe.name} n'est pas disponible à ce moment-là`,
              'error'
            );
          } else if (teacherNotFree) {
            Swal.fire(
              'Erreur',
              `Professeur ${teacher.name} n'est pas disponible à ce moment-là`,
              'error'
            );
          } else if (classroomNotFree) {
            Swal.fire(
              'Erreur',
              `${classroom.name} n'est pas disponible à ce moment-là`,
              'error'
            );
          } else {
            try {
            ...
            <div className="form-group">
              ...
              <div className="input-group mb-3">
                <select
                  ...
                  disabled
                >
                  <option value={inputs.weekday}>
                    {dayToArabic(inputs.weekday)}
                  </option>
                </select>
              </div>
            </div>
